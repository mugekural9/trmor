...
def config():
    ...
    args.device = 'cuda'
    model_id = 'charlm_4_probe_tense'
    model_path, (surf_vocab, tense_vocab)  = get_model_info(model_id) 
    # logging
    args.logdir = 'evaluation/visualization/probe/tense/results/'+model_id+'/'
    ...
    # load vocab (to initialize the model with correct vocabsize)
    with open(tense_vocab) as f:
        word2id = json.load(f)
        args.tense_vocab = VocabEntry(word2id)
    args.vocab = (args.surf_vocab, args.tense_vocab)
    ...
    args.model = CharLM_Lstm_Probe(args, args.tense_vocab, model_init, emb_init)
    ...
    args.model.eval()
    # data
    #args.tstdata = 'evaluation/probing/tense/data/tense.uniquesurfs.trn.txt'#'evaluation/visualization/probe/tense/data/surf.uniquesurfs.trn.txt'
    args.tstdata = 'evaluation/visualization/probe/tense/data/surf.uniquesurfs.trn.txt'
...
def main():
    ...
    with torch.no_grad():
        ...
        for data in batches:
            ...
            label_colors.append(args.color_dict[str(tense.item())])
    ...
    for cid, color in args.color_dict.items(): 
        if color not in label_colors:
            continue 
        color_patch = mpatches.Patch(color=color, label=args.tense_vocab.id2word(int(cid)))
        color_handles.append(color_patch)
    ax.legend(handles=color_handles)
    sc = ax.scatter3D(scores_tensor[:,1],scores_tensor[:,2], scores_tensor[:,3],  color=label_colors)
