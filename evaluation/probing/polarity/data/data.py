...
number_of_polar_tokens = 0; number_of_polar_unks = 0
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')   

def read_trndata_makevocab(args, surf_vocab):
    data = []; surf_data = []; polar_data = []
    ...
    assert len(surf_data) == len(polar_data)
    for (surf, polar) in zip(surf_data,  polar_data):
        data.append([surf, [polar]])
    return  data, VocabEntry(polar_vocab)
    
def read_valdata(maxdsize, file, vocab, mode):
    surf_vocab, polar_vocab = vocab
    data = []; surf_data = []; polar_data = []
    count = 0
...
def get_batch(x, vocab):
    ...
    surface_vocab,  polar_vocab = vocab
    surf, polar = [], []
    max_surf_len = max([len(s[0]) for s in x])
    for surf_idx, polar_idx in x:
        ...
        number_of_polar_tokens += len(polar_idx);  number_of_polar_unks += polar_idx.count(polar_vocab['<unk>'])
...
def get_batches(data, vocab, batchsize=64, seq_to_no_pad=''):
    ...
    number_of_polar_tokens = 0
    number_of_polar_unks = 0
...
def build_data(args, surf_vocab):
    ...
    trndata, pos_vocab = read_trndata_makevocab(args, surf_vocab) 
    vocab = (surf_vocab, pos_vocab)
