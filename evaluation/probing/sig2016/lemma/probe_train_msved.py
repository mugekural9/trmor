...
def test(batches, args, epc):
    ...
    for j, lidx in enumerate(indices):
        ...
        mu, logvar, encoder_fhs = args.model.encoder(surf)
        if probe_part == 'continuous':
            ...
            batch_loss, (acc,pred_tokens, pred_words)  = args.probe.probe_loss(concat_tag, lemma, args.lemma_vocab, args.surf_vocab, surf)
...
def train(data, args):
    ...
    for epc in range(args.epochs):
        ...
        selections_freq = dict()       
        for i in range(len(args.model.tag_embeddings)):
            selections_freq[i] = defaultdict(lambda:0)
