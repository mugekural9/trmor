...
from evaluation.probing.sig2016.lemma_probe import Probe
    ...
    for j, lidx in enumerate(indices):
        ...
        if probe_part == 'quantized':
            ...
            for vq_layer in args.model.ord_vq_layers:
                ...
                quantized_input, vq_loss, quantized_inds, _ = vq_layer(None, bck[:,:,i*args.model.orddict_emb_dim:(i+1)*args.model.orddict_emb_dim],epc)
                vq_vectors.append(quantized_input)
                i+=1
            quantized_vec = torch.cat(vq_vectors,dim=2)
            batch_loss, (acc,pred_tokens, pred_words) = args.probe.probe_loss(quantized_vec, lemma, args.lemma_vocab, args.surf_vocab, surf)
        elif probe_part == 'continuous':
            batch_loss, (acc,pred_tokens, pred_words) = args.probe.probe_loss(_root_fhs, lemma, args.lemma_vocab, args.surf_vocab, surf)

        true_preds, false_preds = pred_words
...
def train(data, args):
    ...
    for epc in range(args.epochs):
        ...
        for j, lidx in enumerate(ltgtindices):
            ...
            surf, tags, lemma  = lxtgt_ordered_batches[lidx] 
            fhs, _, _, mu, logvar, fwd,bck = args.model.encoder(surf)
            _root_fhs = torch.permute(mu.unsqueeze(0), (1,0,2)).contiguous()
        ...
        with open('trues.txt', 'w') as true_writer:
            for true in true_preds:
                true_writer.write(true+'\n')
        with open('falses.txt', 'w') as false_writer:
            for false in false_preds:
                false_writer.write(false+'\n')
...
_model_path, surf_vocab, _  = get_model_info('hungarian_vqvae_probe') 
# initialize model
...
args.maxtrnsize = 1000000; args.maxvalsize = 10000; args.maxtstsize = 1000000000000; args.maxusize=1000000




#if args.lang == 'turkish':
from evaluation.probing.sig2016.lemma.hun_lemma_data import build_data



rawdata, batches, _, tag_vocabs, lemma_vocab = build_data(args, args.surf_vocab)
args.lemma_vocab = lemma_vocab
with open('lemma_vocab.json', 'w') as f:
    f.write(json.dumps(args.lemma_vocab.id2word_, ensure_ascii=False))
