...
def read_trndata_makevocab(args, surface_vocab):
    surf_data = []; feat_data = []; pos_data = []; root_data = [];  data = []; polar_data = []; tense_data = []
    ...
    with open(args.trndata, 'r') as reader:
        ...
        pos_vocab['<unk>'] = 0; polar_vocab['<unk>'] = 0; tense_vocab['<unk>'] = 0
        count = 0
        for line in reader: 
            count += 1
    ...
    assert len(surf_data) == len(feat_data) == len(root_data) == len(pos_data) == len(polar_data) == len(tense_data) 
    for (surf, feat, pos, root, polar, tense) in zip(surf_data, feat_data, pos_data, root_data, polar_data, tense_data):
        data.append([surf, feat, [pos], root, [polar], [tense]])
    vocab = (surface_vocab, VocabEntry(feature_vocab), VocabEntry(pos_vocab), VocabEntry(polar_vocab), VocabEntry(tense_vocab))
...
def build_data(args):
    ...
    tst_batches, _ = get_batches(tstdata, vocab, args.batchsize, 'root') 
...
def log_data(data, dset, vocab, logger, modelname, dsettype='trn'):
    ...
    pos_tags, polar_tags, tense_tags = defaultdict(lambda: 0), defaultdict(lambda: 0), defaultdict(lambda: 0)
    for (surf, feat, pos, root, polar, tense) in data:
