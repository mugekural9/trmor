...
def get_batch_tagmapping(x, surface_vocab, device=device):
    ...
    pos=[]; per=[]; num=[]; tense=[]; aspect=[];  voice=[]; finite=[]; comp=[]; entry = [] 
    rsurf =[];
    max_surf_len = max([len(s[0]) for s in x])
    max_rsurf_len = max([len(s[-1]) for s in x])
    for surf_idx, case_idx,polar_idx, mood_idx ,pos_idx,per_idx,num_idx,tense_idx,aspect_idx,\
    voice_idx, finite_idx, comp_idx, \
    entry_idx, rsurf_idx  in x:
...
comp_vocab=None
    ...
    with open(args.logdir+mode+'_data.txt', 'w') as writer:
        for surf, case,polar,mood,pos,per,num,tense,aspect, voice, finite, comp, entry, rsurf in zip(surf_data, 
        tag_data['case'], 
        tag_data['polar'],
        tag_data['mood'],
        tag_data['pos'],
        ...
        tag_data['finite'],
        tag_data['comp'], 
        entry_data, rsurf_data):
            data.append([surf, case,polar,mood,pos,per,num,tense,aspect, voice, finite, comp, entry, rsurf])
            writer.write(str([surf, case,polar,mood,pos,per,num,tense,aspect, voice, finite, comp, entry, rsurf])+"\n")
...
def build_data(args, surface_vocab=None):
    ...
    tstdata, _ = read_data(args.maxtstsize, args.valdata, surface_vocab, 'TST',
    tag_vocabs['case'],
    tag_vocabs['polar'],
    tag_vocabs['mood'],
    tag_vocabs['pos'],
    ...
    tst_batches, _ = get_batches(tstdata, surface_vocab, 1, args.seq_to_no_pad) 
...
def config():
    ...
    args.device = 'cuda'
    model_id = 'batchsize128_beta_0.2_6x6_bi_kl_0.2_epc190'
    args.lang ='finnish'
    ...
    if 'semisup' in model_id:
        ...
        for tag_vocab in tag_vocabs:
            with open(tag_vocab) as f:
                ...
                j+=1
        args.model = VQVAE(args, args.vocab, args.tag_vocabs, model_init, emb_init, dict_assemble_type='sum_and_concat')
...
args.maxtrnsize = 100000; args.maxvalsize = 1000; args.maxtstsize = 100000
...
def train(args, batches, valbatches, model, vqvae, optimizer, epc):
    ...
    for _, idx in enumerate(indices):
        # (batchsize, t)
        surf, case,polar,mood,pos,per,num,tense,aspect,voice,finite,comp, entry, rsurf= batches[idx] 
        fhs, _, _, mu, logvar, fwd,bck = vqvae.encoder(surf)
        batchsize=surf.size(0)
        ...
        feature_vec = torch.cat((_root_fhs.detach(),
                            ...
                            model.embeddings[3].weight[pos],
                            model.embeddings[4].weight[per],
                            model.embeddings[5].weight[num],
                            model.embeddings[6].weight[tense],
                            model.embeddings[7].weight[aspect],
                            model.embeddings[8].weight[voice],
                            model.embeddings[9].weight[finite],
                            model.embeddings[10].weight[comp]
                            ),dim=1)
...
def test(args, batches, model, vqvae, epc, log_shared_task=False):
    ...
    if log_shared_task:
        args.logger.write('\nTST reinf acc: %.4f' % (true/args.tstsize))
        return epoch_loss, (true/args.tstsize), reinflects, mistakes
...
try:
    ...
    for epc in range(args.epochs):
        val_epoch_loss, val_exact_acc = train(args, trnbatches, valbatches, model, vqvae, optimizer,epc)
        if val_epoch_loss < best_loss:
            args.logger.write('\nupdate best loss \n')
            best_loss = val_epoch_loss    
    
        if False:#epc%20 == 0 or epc>100:
            with torch.no_grad():
