...
class VQVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.lemmadict_emb_num = args.lemmadict_emb_num
        
        self.dict_assemble_type = dict_assemble_type
        
     
        if dict_assemble_type == 'sum':
            self.orddict_emb_dim = self.rootdict_emb_dim
        else:
            self.orddict_emb_dim   = int(args.enc_nh/(self.num_dicts-1))
            self.lemmadict_emb_dim = args.enc_nh
           
        self.beta = args.beta

        self.vq_layer_lemma = VectorQuantizer(self.lemmadict_emb_num,
                                        self.lemmadict_emb_dim,
                                        self.beta)
    ...
    def vq_loss(self,x, epc):
        # fhs: (B,1,hdim)
        fhs, _, _, fwd,bck = self.encoder(x)
        
        vq_vectors = []; vq_losses = []; vq_inds = []
        # quantized_input: (B, 1, rootdict_emb_dim)
        
        
        quantized_input, vq_loss, quantized_inds = self.vq_layer_lemma(fwd,epc)
