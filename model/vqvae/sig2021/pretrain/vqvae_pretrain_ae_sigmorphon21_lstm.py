...
from model.vqvae.data.data_sigmorphon2021 import build_data
    ...
    for epc in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if loss < best_loss:
                ...
                if args.model.encoder.lstm.bidirectional:
                    torch.save(epoch_encoder_fhs_fwd, 'model/vqvae/results/fhs/SIG2021/'+args.lang+'_fhs_SIGMORPHON2021-LSTM-train_fwd_d'+str(args.enc_nh)+'.pt')
                    torch.save(epoch_encoder_fhs_bck, 'model/vqvae/results/fhs/SIG2021/'+args.lang+'_fhs_SIGMORPHON2021-LSTM-train_bck_d'+str(args.enc_nh)+'.pt')
                    torch.save(epoch_encoder_fhs,     'model/vqvae/results/fhs/SIG2021/'+args.lang+'_fhs_SIGMORPHON2021-LSTM-train_all_d'+str(args.enc_nh*2)+'.pt')
...
#tensorboard
writer = SummaryWriter("runs/pretraining_ae/dataset-sig2021/"+args.lang+'/')


# logging
args.modelname = 'model/'+args.mname+'/results/training/'+args.lang+'/sig2021/lstm/'+str(len(trndata))+'_instances/enc_nh_' + str(args.enc_nh) + '/' 
try:
