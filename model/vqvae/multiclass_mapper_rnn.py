...
# data
args.trndata  = 'trn.txt'
args.valdata  = 'val.txt'
...
class Tagmapper(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=self.emb_dim,
                            ...
                            num_layers=1,
                            batch_first=True)
        self.pred_linear = nn.Linear(self.rnn_dim, 25, bias=False)
...
for epc in range(150):
    ...
    for i, idx in enumerate(indices):
        ...
        feature_vec = torch.cat((_root_fhs.detach(),
                            ...
                            model.linears[10].weight[poss]),dim=1)
        # ([60, 12, 128])
        _, (last_state, last_cell) = model.lstm(feature_vec)
         # (batch_size, 1, enc_nh)
        last_state = last_state.permute(1,0,2)

        # 1,1,25
        logits = model.pred_linear(last_state)
        # 128,1,25
        probs_1 = sft(logits)
