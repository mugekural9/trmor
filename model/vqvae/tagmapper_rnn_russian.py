...
def get_batch_tagmapping(x, surface_vocab, device='cuda'):
    ...
    return  torch.tensor(surf, dtype=torch.long,  requires_grad=False, device=device), \
            ...
            torch.tensor(anim, dtype=torch.long, requires_grad=False, device=device), \
...
def read_data(maxdsize, file, surface_vocab, mode, 
    ...
    if anim_vocab is None:
        anim_vocab = defaultdict(lambda: len(anim_vocab))
        anim_vocab['<pad>'] = 0
    tag_vocabs['anim'] = anim_vocab
    tag_data['anim'] = []
...
def config():
    ...
    if 'late' in model_id:
        ...
        args.emb_dim = 512 #128#args.dec_nh
        j=0
...
#tag_analysis(args.model_id, args.num_dicts, args.lang, args.logger)
...
def train(args, lxsrc_ordered_batches, lxtgt_ordered_batches, valbatches, model, vqvae, optimizer, epc):
    ...
    for _, idx in enumerate(lxsrc_indices):
        ...
        ## surf-> rsurf
        # (batchsize, t)
        surf, case,polar,mood,evid,pos,per,num,tense,aspect,inter,poss, junk, entry, rsurf= lxsrc_ordered_batches[idx] 
        ...
        feature_vec = torch.cat((_root_fhs.detach(),
                            ...
                            model.embeddings[10].weight[poss],
                            model.embeddings[11].weight[junk]),dim=1)
...
def test(args, batches, model, vqvae, epc, log_shared_task=False):
    ...
    for _, idx in enumerate(indices):
        # (batchsize, t)
        surf, case,polar,mood,evid,pos,per,num,tense,aspect,inter,poss,junk, entry, rsurf = batches[idx] 
        ...
        if log_shared_task:
            ...
            if args.num_dicts >=12:
                vq_vectors.append(vqvae.ord_vq_layers[11].embedding.weight[reinflect_tag[11]].unsqueeze(0).unsqueeze(0))
