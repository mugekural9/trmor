...
from model.vqvae.sig2016.early_sup.vqvae_kl_bi_early_sup_nobias_lstm import VQVAE
...
def shared_task_gen(args,epc):
    ...
    with open(args.tstdata, 'r') as reader:
        ...
        for line in reader:
            ...
            oracle_keys = oracle(args, gold_reinflection)
...
parser.add_argument('--kl_max', type=float, required=True)
...
args.model_prefix = 'run'+str(args.run_id)+'_batch'+str(args.batchsize)+'_beta'+str(args.beta)+'_bi_kl'+str(args.kl_max)+'_'+str(args.num_dicts)+"x"+str(args.orddict_emb_num)+'_dec'+str(args.dec_nh)+'_suffixd'+str(args.incat)+'/'
writer = SummaryWriter("runs/sig2016/early-sup/nobias-lstm/"+args.lang+'/'+ args.model_prefix)
...
# logging
args.modelname = 'model/'+args.mname+'/results/training/sig2016/'+args.lang+'/early-sup/nobias-lstm/'+str(args.lxtgtsize)+'_instances/'+args.model_prefix
