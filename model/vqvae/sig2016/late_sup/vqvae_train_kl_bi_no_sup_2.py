...
def train(data, args):
    ...
    for epc in range(args.epochs):
        ...
        for i, lidx in enumerate(ltgtindices):
            ...
            lxtgt_loss, lxtgt_recon_loss, lxtgt_vq_loss, (lxtgt_acc,pred_tokens), quantized_inds,  encoder_fhs, vq_codes_list, suffix_code_list, recon_preds, lxtgt_kl_loss, logdetmaxloss = args.model.loss(lxtgt, None, kl_weight, epc)
        ...
        args.logger.write('\nTRN')
        args.logger.write('\nlxtgt_loss: %.4f, lxtgt_vq_loss: %.4f, lxtgt_kl_loss: %.4f, lxtgt_recon_loss: %.4f, ltxtgt_recon_acc: %.4f' % (lxtgt_loss, lxtgt_vq, lxtgt_kl, lxtgt_recon, lxtgt_acc))
    ...
    with open(args.tstdata, 'r') as reader:
        ...
        for line in reader:
            ...
            if reinflected_word == gold_reinflection:
                true +=1
                true_ones.append(inflected_word +'\t'+gold_reinflection + '\t'+reinflected_word+'\t'+ '-'.join([str(s) for s in suffix_code])+'\n')
            else:
                false_ones.append(inflected_word +'\t'+gold_reinflection + '\t'+reinflected_word+'\t'+ '-'.join([str(s) for s in suffix_code])+'\n')
    acc = (true/i)
...
args.model_prefix = 'run'+str(args.run_id)+'_batchsize'+str(args.batchsize)+'_beta'+str(args.beta)+'_bi_kl'+str(args.kl_max)+'_'+str(args.num_dicts)+"x"+str(args.orddict_emb_num)+'_dec'+str(args.dec_nh)+'_suffixd'+str(args.incat)+'/'
writer = SummaryWriter("runs/late-sup-2/"+args.lang+'/'+ args.model_prefix)
...
# logging
args.modelname = 'model/'+args.mname+'/results/training/sig2016/'+args.lang+'/late-sup-2/'+str(args.trnsize)+'_instances/'+args.model_prefix
