...
if dataset_type == 'V':
    if args.lang == 'georgian':
        ...
        _model_id = 'ae_'+args.lang+'_unsup_360'
        #ae_fhs_vectors_bck = torch.load('model/vqvae/results/fhs/'+args.lang+'_fhs_datasetV-train_bck_d240.pt').to('cpu')
        ae_fhs_vectors= torch.load('model/vqvae/results/fhs/'+args.lang+'_fhs_SIGMORPHON2016-train_all_d720.pt').to('cpu')
...
args.model_prefix =  'run'+str(args.run_id)+'_upnum'+str(args.upnum)+'_do'+str(args.dec_dropout_in)+'_kl_decay'+str(args.kl_decay)+'_nz'+str(args.nz)+'_batchsize'+str(args.batchsize)+'_beta'+str(args.beta)+'_bi_kl'+str(args.kl_max)+'_'+str(args.num_dicts)+"x"+str(args.orddict_emb_num)+'_dec'+str(args.dec_nh)+'_suffixd'+str(args.incat)+'/'
writer = SummaryWriter("runs/no-supervision-nobias-lstm/"+args.lang+'/'+ args.model_prefix)
...
# logging
args.modelname = 'model/'+args.mname+'/results/training/sig2016/'+args.lang+'/late-sup/nobias-lstm/'+str(args.usize)+'_instances/'+args.model_prefix
