...
def get_batch_tagmapping(x, surface_vocab, device='cuda'):
    ...
    #['alt', 'polite', 'gen', 'polar', 'mood', 'pos', 'per', 'num', 'tense', 'aspect', 'finite']
...
def build_data(args, surface_vocab, tag_vocabs):
    # Read data and get batches...
    #surface_vocab = MonoTextData(args.surface_vocab_file, label=False).vocab
    trndata, _ = read_data(args.maxtrnsize, args.trndata, surface_vocab, 'TRN',
    tag_vocabs['gen'],
    tag_vocabs['polite'],
    tag_vocabs['alt'],
    tag_vocabs['polar'],
...
def config():
    ...
    if 'late' in model_id:
        ...
        keys =  ['alt', 'polite', 'gen', 'polar', 'mood', 'pos', 'per', 'num', 'tense', 'aspect', 'finite']
        for tag_vocab in tag_vocabs:
            with open(tag_vocab) as f:
                word2id = json.load(f)
                args.tag_vocabs[keys[j]] = VocabEntry(word2id) 
                j+=1
...
rawdata, batches, vocab, tag_vocabs = build_data(args, args.surf_vocab, args.tag_vocabs)
...
def train(args, lxsrc_ordered_batches, lxtgt_ordered_batches, valbatches, model, vqvae, optimizer, epc):
    ...
    for _, idx in enumerate(lxsrc_indices):
        ...
        correct += torch.sum(pred.squeeze(1) == entry[:,dict_no]).item()
...
def test(args, batches, model, vqvae, epc, log_shared_task=False):
    ...
    for _, idx in enumerate(indices):
        ...
        if log_shared_task:
            preds.append(''.join(args.surf_vocab.decode_sentence(surf[0]))+'\t'+ str(pred.item()))    

    args.logger.write('\nVAL epoch: %d, epoch_loss: %.4f epoch_acc: %.5f, exact_acc: %.4f' % (epc, epoch_loss, correct/total, exact_correct/args.valsize))
    return epoch_loss, (correct/total), preds
