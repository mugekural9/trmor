...
def test(batches, mode, args, kl_weight):
    ...
    for i, idx in enumerate(indices):
        ...
        # (batchsize)
        loss, recon_loss, kl_loss, recon_acc, encoder_fhs = args.model.loss(surf,case,polar,mood,evid,pos,per,num,tense,aspect,inter,poss, reinflect_surf, kl_weight, mode='test')
...
def train(data, args):
    ...
    for epc in range(args.epochs):
        ...
        for i, idx in enumerate(indices):
            ...
            # (batchsize)
            loss, recon_loss, kl_loss, acc, encoder_fhs = args.model.loss(surf,case,polar,mood,evid,pos,per,num,tense,aspect,inter,poss, reinflect_surf, kl_weight)
...
args.ni = 256; args.nz = 150; 
args.enc_nh = 512; args.dec_nh = 256
