    ...
    if False:#log_shared_task:
        ...
        with open('semisup_tgt_to_src_reinflections.txt','w') as writer:
            for i, idx in enumerate(ltgtindices):
                lxsrc, tags, lxtgt  = lxtgt_ordered_batches[idx] 
    ...
    for epc in range(args.epochs):
        ...
        for i, uidx in enumerate(uindices):
            ...
            if i < len(lxsrc_ordered_batches):
                lidx= lsrcindices[i]
                lxsrc, tags, lxtgt  = lxsrc_ordered_batches[lidx] 
                # MSVAE with lxsrc
                ...
                '''
                # Labeled MSVED 
                loss_labeled_msved, labeled_msved_tag_pred_loss, labeled_msved_tag_correct, labeled_msved_tag_total, labeled_msved_recon_loss, labeled_msved_kl_loss, labeled_msved_recon_acc = args.model.loss_labeled_msved(lxsrc, tags, lxtgt, kl_weight, tmp)
...
def shared_task_gen(batches, args, tmp):
    ...
    with open('semisup_msved_shared_task_tst_beam.txt', 'w') as f:
        for i, idx in enumerate(indices):
            ...
            f.write(inflected_form+'\t'+gold_reinflected_form+ '\t'+direct_reinflected_form+ '\n')
            ...
            f.write('Gumbel hardtrue: ' + inflected_form+'\t'+gold_reinflected_form+ '\t'+gumbel_reinflected_form+ '\n')
    ...
    args.logger.write('\nTST SET ACCURACY with GUMBEL hardtrue: %.3f' % (gumbel_hardtrue_correct/1600))
...
args.device = 'cuda'
# training
args.batchsize = 16; args.epochs = 150
...
args.unlabeled_data = 'data/sigmorphon2016/turkish_ux.txt'
